...
$stmt=$dbh->prepare('select shitumon_id,shitumon_hentou,shitumon_hentou_hiduke,shitumon_hentousha from shitumon where shitumon_id=:target_user_id');
  ...
  $stmt=$dbh->prepare('select shitumon_jukousha,shitumon_jukousha_hentou,shitumon_hiduke from shitumon where shitumon_id=:target_user_id');
   ...
   if ($result = $stmt->fetchAll(PDO::FETCH_ASSOC)) {
    ...
    $shitumon_hentou_hiduke = $row['shitumon_hiduke'];
    $shitumon_jukousha_hentou = $row['shitumon_jukousha_hentou'];

    $shitumon_line2.="<tr><td>".$shitumon_jukousha."</td><td>".$shitumon_hentou_hiduke."</td><td>".$shitumon_jukousha_hentou."</td></tr>\n";
